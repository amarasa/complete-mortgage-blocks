...
function enqueue_jvectormap_scripts()
{
    ...
    // Enqueue the interactive map initialization script.
    wp_enqueue_script(
        'cms-interactive-map',
        plugin_dir_url(__FILE__) . 'blocks/interactive-map/interactive-map.js',
        array('jvectormap-us'),
        '2.99.4',
        true
    );
...
/**
 * Conditionally enqueue block-specific scripts
 */
function cms_enqueue_block_scripts() {
    ...
    if (has_block('acf/cms-statistics', $content)) {
        wp_enqueue_script(
            'cms-statistics',
            plugin_dir_url(__FILE__) . 'blocks/statistics/statistics.js',
            array(),
            '2.99.4',
            true
        );
    ...
    // FAQ Accordion block — loaded via block.json viewScript
}
