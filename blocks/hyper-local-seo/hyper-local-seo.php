                ...
                <?php foreach ($list as $item): ?>
                    <div class="col-span-12 md:col-span-6 lg:col-span-4 mb-4">
                        <?php if ($use_hyperlinks && !empty($item['link_item'])): ?>
                            <a href="<?php echo esc_url($item['link_item']['url']); ?>" target="<?php echo esc_attr($item['link_item']['target'] ?: '_self'); ?>" class="text-secondary !text-2xl font-bold hover:text-primary duration-300 ease-in-out transition-all !no-underline block text-center">
                                <?php echo esc_html($item['link_item']['title']); ?>
                            </a>
                        <?php else: ?>
                            <p class="text-center font-medium"><?php echo esc_html($item['list_item']); ?></p>
                        <?php endif; ?>
                    </div>
                <?php endforeach; ?>
            </div>
